...
class TwitterClient(context: Context) : OAuthBaseClient(
    ...
    companion object {
        val REST_API_INSTANCE = TwitterApi.instance() // Initialize connection to Twitter API
    ...
    fun getHomeTimeline(handler: JsonHttpResponseHandler) {
        ...
        params.put("count", "25") // for retrieving 25 tweets each request
        params.put("since_id", 1) // setting to 1 means no filtering, giving newest tweets
    ...
    fun publishTweet(tweetContent: String, handler: JsonHttpResponseHandler) {
        ...
        // Can specify query string params directly or through RequestParams.
        val params = RequestParams()
        params.put("status", tweetContent) // for retrieving 25 tweets each request
